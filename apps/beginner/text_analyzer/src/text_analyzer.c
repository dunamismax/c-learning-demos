 ...
 * @brief Text analysis tool demonstrating file I/O, string processing, and
 * statistics
 * @author dunamismax
 * @date 2025
 *
 ...
 */
bool is_sentence_ending(char c) { return c == '.' || c == '!' || c == '?'; }
 ...
 */
void analyze_line(const char *line, TextStats *stats,
                  WordFrequencyTable *word_table, int line_number) {
  if (!line || !stats || !word_table)
    return;
 ...
 */
bool analyze_text_file(const char *filename, TextStats *stats,
                       WordFrequencyTable *word_table) {
  if (!filename || !stats || !word_table)
    return false;
  ...
  if (stats->total_words > 0) {
    stats->avg_word_length =
        (double)stats->alphabetic_chars / stats->total_words;
  }
  ...
  if (stats->total_paragraphs > 0) {
    stats->avg_paragraph_length =
        (double)stats->total_words / stats->total_paragraphs;
  }
...
void generate_frequency_report(WordFrequencyTable *word_table, int top_n) {
  if (!word_table || top_n <= 0)
    return;

  // Create array of word frequencies
  WordFrequency *frequencies =
      malloc(word_table->total_entries * sizeof(WordFrequency));
  if (!frequencies) {
  ...
  // Sort by frequency
  quick_sort(frequencies, word_table->total_entries, sizeof(WordFrequency),
  ...
  int display_count =
      (top_n < word_table->total_entries) ? top_n : word_table->total_entries;
...
void display_text_statistics(const TextStats *stats, const char *filename) {
  ...
  if (stats->total_sentences > 0) {
    ...
    double complexity_score =
        avg_words_per_sentence * stats->avg_word_length / 10.0;
