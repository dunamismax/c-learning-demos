 ...
 * @brief Secure password generator demonstrating string manipulation and
 * randomization
 * @author dunamismax
 * @date 2025
 *
 ...
 */
int build_charset(const PasswordOptions *options, char *charset,
                  size_t charset_size) {
  if (!options || !charset || charset_size == 0)
    return 0;
 ...
 */
bool generate_password(const PasswordOptions *options, char *password,
                       size_t password_size) {
  if (!options || !password || password_size == 0)
    return false;
 ...
 */
void analyze_password_strength(const char *password,
  ...
  if (analysis->has_special)
    charset_size += strlen(SPECIAL_CHARS);
  ...
  if (analysis->unique_chars >= length * 0.8)
    analysis->score += 10;
  else if (analysis->unique_chars >= length * 0.6)
    analysis->score += 5;
  ...
  if (analysis->score >= 80) {
    ...
    analysis->strength_description = "Very Weak";
    analysis->recommendations =
        "Password is extremely weak, please generate a new one";
  }
}
...
bool get_password_length(PasswordOptions *options) {
  ...
  printf("Enter password length (%d-%d): ", MIN_PASSWORD_LENGTH,
         MAX_PASSWORD_LENGTH);
  ...
  while (true) {
    ...
    if (str_to_int(trim_whitespace(input), &length)) {
      if (length >= MIN_PASSWORD_LENGTH && length <= MAX_PASSWORD_LENGTH) {
        ...
        printf("Please enter a length between %d and %d: ", MIN_PASSWORD_LENGTH,
               MAX_PASSWORD_LENGTH);
      }
    } else {
