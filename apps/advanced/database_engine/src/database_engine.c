...
bool read_page(DatabaseEngine* db, uint32_t page_id, Page* page) {
    if (!db || !page || db->db_fd < 0) return false;
    ...
    if (page->header.checksum != 0 && page->header.checksum != calculated_checksum) {
...
bool create_table(DatabaseEngine* db, const char* table_name, 
    ...
    if (!write_page(db, &root_page)) {
        log_message("ERROR", "Failed to create root page for table '%s'", table_name);
        return false;
    }
    
    db->table_count++;
    
    log_message("INFO", "Created table '%s' with %zu columns", table_name, column_count);
    return true;
}
...
uint32_t insert_record(DatabaseEngine* db, const char* table_name, 
    ...
    if (!db || !table_name || !values) return 0;
    ...
    if (db->debug_mode) {
        log_message("DEBUG", "Inserted record %u into table '%s'", record_id, table_name);
    }
    ...
    for (size_t i = 0; i < db->table_count; i++) {
        ...
        for (size_t j = 0; j < table->column_count; j++) {
            ...
            const char* type_name = 
                col->type == TYPE_INTEGER ? "INTEGER" :
                col->type == TYPE_STRING ? "STRING" :
                col->type == TYPE_DOUBLE ? "DOUBLE" :
                col->type == TYPE_BOOLEAN ? "BOOLEAN" : "UNKNOWN";
            
            char flags[16] = "";
            if (col->is_primary_key) strcat(flags, "PK ");
            if (col->is_nullable) strcat(flags, "NULL");
            
            printf("  %-20s %-12s %-8zu %-8s\n", 
                   col->name, type_name, col->size, flags);
        }
        printf("\n");
    }
...
void show_statistics(DatabaseEngine* db) {
    ...
    printf("Transaction state: %s\n", 
           ...
           db->current_transaction_state == TRANSACTION_COMMITTED ? "COMMITTED" : "ABORTED");
    ...
    for (size_t i = 0; i < BUFFER_POOL_SIZE; i++) {
        ...
        if (entry->page) {
            used_buffers++;
            if (entry->is_dirty) dirty_buffers++;
            if (entry->is_pinned || entry->pin_count > 0) pinned_buffers++;
        }
    }
...
void process_command(DatabaseEngine* db, const char* command) {
    ...
    if (strcmp(cmd, "CREATE") == 0) {
        ...
        Column columns[] = {
            {"id", TYPE_INTEGER, sizeof(int), true, false},
        ...
        if (!into_keyword || !table_name || !values_keyword ||
            strcmp(into_keyword, "INTO") != 0 || strcmp(values_keyword, "VALUES") != 0) {
        ...
        printf("Supported commands: CREATE TABLE, INSERT INTO, SELECT FROM, SHOW\n");
    }
}
...
void run_interactive_mode(DatabaseEngine* db) {
    ...
    while (true) {
        ...
        if (strcmp(command, "help") == 0) {
            ...
            printf("  CREATE TABLE <name>         - Create table with default columns\n");
