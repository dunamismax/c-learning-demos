...
bool compiler_consume(Compiler *compiler, TokenType type, const char *message) {
  ...
  snprintf(compiler->error_message, sizeof(compiler->error_message),
           "%s at line %d. Got '%s'", message, compiler->current_token.line,
           compiler->current_token.lexeme);
  return false;
}
...
ASTNode *parse_primary(Compiler *compiler) {
  ...
  if (compiler_check(compiler, TOKEN_STRING)) {
    ...
    if (node) {
      strcpy(node->data.literal.value.string,
             compiler->current_token.value.string);
      node->line = compiler->current_token.line;
      node->column = compiler->current_token.column;
    }
    compiler_advance(compiler);
  ...
  if (compiler_check(compiler, TOKEN_TRUE) ||
      compiler_check(compiler, TOKEN_FALSE)) {
  ...
  if (compiler_check(compiler, TOKEN_LEFT_PAREN)) {
    ...
    if (!compiler_consume(compiler, TOKEN_RIGHT_PAREN,
  ...
  snprintf(compiler->error_message, sizeof(compiler->error_message),
           "Unexpected token '%s' at line %d", compiler->current_token.lexeme,
...
ASTNode *parse_unary(Compiler *compiler) {
  ...
  if (compiler_check(compiler, TOKEN_NOT) ||
      compiler_check(compiler, TOKEN_MINUS)) {
...
ASTNode *parse_term(Compiler *compiler) {
  ...
  while (compiler_check(compiler, TOKEN_PLUS) ||
         compiler_check(compiler, TOKEN_MINUS)) {
    ASTNode *binary = ast_create_node(AST_BINARY_EXPRESSION);
    if (!binary) {
      ast_free_node(expr);
...
ASTNode *parse_equality(Compiler *compiler) {
  ...
  while (compiler_check(compiler, TOKEN_EQUAL) ||
         compiler_check(compiler, TOKEN_NOT_EQUAL)) {
    ASTNode *binary = ast_create_node(AST_BINARY_EXPRESSION);
    if (!binary) {
      ast_free_node(expr);
      return NULL;
...
ASTNode *parse_var_declaration(Compiler *compiler) {
  ...
  if (!compiler_consume(compiler, TOKEN_SEMICOLON,
...
ASTNode *parse_program(Compiler *compiler) {
  ...
  if (stmt_count > 0) {
    program->data.program.statements =
        safe_calloc(stmt_count, sizeof(ASTNode *));
...
int main(int argc, char *argv[]) {
  ...
  if (source_file) {
    ...
    const char *demo_programs[] = {"let x = 42;\nprint x;",
                                   "let name = \"World\";\nprint name;",
                                   "let result = 10 + 5 * 2;\nprint result;",
                                   "let a = true;\nlet b = false;\nprint a;"};
