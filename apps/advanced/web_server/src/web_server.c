/**
 * @file web_server.c
 * @brief HTTP web server demonstrating network programming and protocol implementation
 * @author dunamismax
 * @date 2025
 * 
 ...
 */
typedef void (*RouteHandler)(const HTTPRequest* request, HTTPResponse* response);
 ...
 */
bool http_response_add_header(HTTPResponse* response, const char* name, const char* value) {
 ...
 */
bool http_response_set_body(HTTPResponse* response, const char* body, size_t length) {
...
bool parse_http_request(const char* raw_request, HTTPRequest* request) {
    if (!raw_request || !request) return false;
 ...
 */
size_t build_http_response(const HTTPResponse* response, char* buffer, size_t buffer_size) {
    ...
    if (response->body && response->body_length > 0) {
        ...
        size_t body_to_copy = response->body_length < body_space ? 
                             response->body_length : body_space;
 ...
 */
void serve_static_file(WebServer* server, const HTTPRequest* request, HTTPResponse* response) {
    if (!server || !request || !response) return;
    
    // Build file path
    char file_path[1024];
    snprintf(file_path, sizeof(file_path), "%s%s", server->document_root, request->url);
    ...
    if (!file) {
        ...
        strcpy(response->status_message, http_status_message(HTTP_500_INTERNAL_SERVER_ERROR));
...
void handle_root(const HTTPRequest* request, HTTPResponse* response) {
    ...
    const char* html = 
        ...
        "            <p>This is a demonstration web server built in C, showcasing:</p>\n"
        "            <ul>\n"
        ...
        "            <div class=\"endpoint\">\n"
        "                <span class=\"method\">GET</span> / - This welcome page\n"
        "            </div>\n"
        "            <div class=\"endpoint\">\n"
        "                <span class=\"method\">GET</span> /status - Server status information\n"
        "            </div>\n"
        "            <div class=\"endpoint\">\n"
        "                <span class=\"method\">GET</span> /api/time - Current server time (JSON)\n"
        "            </div>\n"
        "            <div class=\"endpoint\">\n"
        "                <span class=\"method\">GET</span> /api/stats - Server statistics (JSON)\n"
        "            </div>\n"
        "        </div>\n"
        "    </div>\n"
        "</body>\n"
...
void handle_api_stats(const HTTPRequest* request, HTTPResponse* response) {
    ...
    snprintf(json, sizeof(json),
        ...
        "  \"errors_5xx\": %zu\n"
        "}",
        g_server->stats.total_requests,
        g_server->stats.total_responses,
        g_server->stats.bytes_sent,
        g_server->stats.bytes_received,
        g_server->stats.active_connections,
        g_server->stats.total_connections,
        uptime,
        g_server->stats.errors_4xx,
        g_server->stats.errors_5xx);
...
bool web_server_init(WebServer* server, int port, const char* document_root) {
    ...
    server->routes[server->route_count++] = (Route){"/", HTTP_GET, handle_root, "Home page"};
    server->routes[server->route_count++] = (Route){"/status", HTTP_GET, handle_status, "Server status"};
    server->routes[server->route_count++] = (Route){"/api/time", HTTP_GET, handle_api_time, "Current time API"};
    server->routes[server->route_count++] = (Route){"/api/stats", HTTP_GET, handle_api_stats, "Server statistics API"};
...
RouteHandler find_route_handler(WebServer* server, const HTTPRequest* request) {
    ...
    for (size_t i = 0; i < server->route_count; i++) {
        ...
        if (route->method == request->method && strcmp(route->path, request->url) == 0) {
...
void* handle_client_connection(void* arg) {
    ...
    while (server->running) {
        ...
        if (setsockopt(conn->socket_fd, SOL_SOCKET, SO_RCVTIMEO, 
                       &timeout, sizeof(timeout)) < 0) {
            if (server->debug_mode) {
                log_message("DEBUG", "Failed to set socket timeout for %s", conn->ip_address);
            }
            break;
        }
        ...
        if (bytes_received <= 0) {
            if (bytes_received == 0) {
                ...
                if (server->debug_mode) {
                    log_message("DEBUG", "Receive error from %s: %s", 
                               conn->ip_address, strerror(errno));
                }
            }
            break;
        }
        ...
        if (server->debug_mode) {
            log_message("DEBUG", "%s %s from %s", 
                       http_method_string(request.method), request.url, conn->ip_address);
        }
        ...
        size_t response_length = build_http_response(&response, response_buffer, sizeof(response_buffer));
        
        if (response_length > 0) {
            ssize_t bytes_sent = send(conn->socket_fd, response_buffer, response_length, 0);
...
bool web_server_start(WebServer* server) {
    ...
    if (listen(server->server_fd, MAX_CONNECTIONS) < 0) {
        log_message("ERROR", "Failed to listen on server socket: %s", strerror(errno));
        close(server->server_fd);
        return false;
    }
    ...
    while (server->running) {
        ...
        int activity = select(server->server_fd + 1, &read_fds, NULL, NULL, &timeout);
        ...
        inet_ntop(AF_INET, &client_addr.sin_addr, conn->ip_address, INET_ADDRSTRLEN);
...
void display_help(const char* program_name) {
    ...
    printf("  -d, --document-root <path>  Document root directory (default: ./www)\n");
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (strcmp(argv[i], "-d") == 0 || strcmp(argv[i], "--document-root") == 0) {
            if (++i >= argc) {
