 ...
 * @brief Network client demonstrating socket programming and network
 * communication
 * @author dunamismax
 * @date 2025
 *
 ...
 */
typedef enum { PROTOCOL_TCP, PROTOCOL_UDP, PROTOCOL_HTTP } NetworkProtocol;
 ...
 */
bool network_client_init(NetworkClient *client, const char *hostname, int port,
                         NetworkProtocol protocol) {
  if (!client || !hostname)
    return false;
 ...
 */
bool resolve_hostname(const char *hostname, char *ip_address,
                      size_t buffer_size) {
  if (!hostname || !ip_address || buffer_size == 0)
    return false;
...
bool set_socket_timeout(int socket_fd, int timeout_seconds) {
  if (socket_fd < 0 || timeout_seconds <= 0)
    return false;
...
bool tcp_connect(NetworkClient *client) {
  ...
  if (connect(client->socket_fd, (struct sockaddr *)&client->server_addr,
    ...
    log_message("ERROR", "Failed to connect to %s:%d - %s", client->hostname,
  ...
  log_message("INFO", "Connected to %s:%d via TCP", client->hostname,
              client->port);
  return true;
}
...
bool udp_setup(NetworkClient *client) {
  ...
  log_message("INFO", "UDP socket created for %s:%d", client->hostname,
              client->port);
  return true;
}
 ...
 */
ssize_t network_send(NetworkClient *client, const void *data,
                     size_t data_length) {
 ...
 */
ssize_t network_receive(NetworkClient *client, void *buffer,
                        size_t buffer_size) {
  ...
  if (client->protocol == PROTOCOL_UDP) {
    ...
    bytes_received =
        recvfrom(client->socket_fd, buffer, buffer_size - 1, 0,
...
bool parse_http_response(const char *response_data, HTTPResponse *response) {
  ...
  if (sscanf(response_data, "HTTP/%*s %d %127s", &response->status_code,
             response->status_message) != 2) {
  ...
  if (!headers_end) {
    log_message("ERROR",
                "Invalid HTTP response format - no header/body separator");
    return false;
  }
  ...
  const char *content_length_header =
      strstr(response->headers, "Content-Length:");
  if (content_length_header) {
    sscanf(content_length_header, "Content-Length: %zu",
 ...
 */
bool http_get_request(NetworkClient *client, const char *path,
  ...
  while (total_received < sizeof(response_buffer) - 1) {
    bytes_received = network_receive(client, response_buffer + total_received,
  ...
  log_message("INFO", "Received HTTP response: %d %s", response->status_code,
...
void network_disconnect(NetworkClient *client) {
  ...
  log_message("INFO", "Disconnected from %s:%d", client->hostname,
              client->port);
...
void display_client_stats(NetworkClient *client) {
  ...
  printf("Protocol: %s\n", client->protocol == PROTOCOL_TCP   ? "TCP"
                           : client->protocol == PROTOCOL_UDP ? "UDP"
                                                              : "HTTP");
  printf("Status: %s\n", client->status == CONNECTION_CONNECTED ? "Connected"
                         : client->status == CONNECTION_CONNECTING
                             ? "Connecting"
                         : client->status == CONNECTION_ERROR ? "Error"
                                                              : "Disconnected");
