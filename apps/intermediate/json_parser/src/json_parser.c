...
bool json_parse_number(JSONParser *parser, double *result) {
  ...
  if (json_parser_current_char(parser) == '0') {
    ...
    while (isdigit(json_parser_current_char(parser)) &&
           number_len < sizeof(number_str) - 1) {
  ...
  // Parse exponent part
  if (json_parser_current_char(parser) == 'e' ||
      json_parser_current_char(parser) == 'E') {
    number_str[number_len++] = json_parser_current_char(parser);
    json_parser_advance(parser);

    if (json_parser_current_char(parser) == '+' ||
        json_parser_current_char(parser) == '-') {
      number_str[number_len++] = json_parser_current_char(parser);
      json_parser_advance(parser);
    }
  ...
  while (true) {
    ...
    if (object->value.object.count >= object->value.object.capacity) {
      size_t new_capacity = object->value.object.capacity == 0
                                ? 4
                                : object->value.object.capacity * 2;
