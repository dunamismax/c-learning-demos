/**
 * @file file_manager.c
 * @brief Interactive file manager demonstrating advanced file I/O and system programming
 * @author dunamismax
 * @date 2025
 * 
...
const char* get_file_type_string(FileType type) {
    switch (type) {
        ...
        case FILE_TYPE_CHAR_DEVICE: return "Char Device";
...
int compare_files(const void* a, const void* b) {
    ...
    // Directories first
    if (file1->type == FILE_TYPE_DIRECTORY && file2->type != FILE_TYPE_DIRECTORY) {
        return -1;
    }
    if (file1->type != FILE_TYPE_DIRECTORY && file2->type == FILE_TYPE_DIRECTORY) {
        return 1;
        ...
        if (darray_get(fm->file_list, i, &info)) {
            if (fm->show_details) {
                ...
                printf("%-11s %-8s %-8s %-10s %-20s %s%s\n",
                       permissions, info.owner_name, info.group_name, size_str, time_str,
                       info.name, (info.type == FILE_TYPE_DIRECTORY) ? "/" : "");
            } else {
                printf("%s%s\n", info.name, (info.type == FILE_TYPE_DIRECTORY) ? "/" : "");
            }
        }
...
bool change_directory(FileManager* fm, const char* path) {
    ...
    if (strcmp(path, "~") == 0) {
        ...
        // Relative path
        snprintf(resolved_path, sizeof(resolved_path), "%s/%s", fm->current_directory, path);
    }
...
bool process_command(FileManager* fm, const char* command) {
    ...
    if (strcmp(cmd, "mkdir") == 0) {
        ...
        if (name[0] == '/') {
            ...
            snprintf(full_path, sizeof(full_path), "%s/%s", fm->current_directory, name);
        }
