 ...
 */
bool memory_pool_init(MemoryPool *pool, size_t size, size_t alignment,
                      const char *name) {
  if (!pool || size == 0)
    return false;
  ...
  log_message("INFO", "Memory pool '%s' initialized: %zu bytes", pool->name,
              size);
  return true;
}
...
bool validate_block(BlockHeader *block) {
  ...
  return (block->signature == BLOCK_SIGNATURE &&
          block->magic == BLOCK_SIGNATURE && block->size > 0);
}
 ...
 */
BlockHeader *split_block(MemoryPool *pool, BlockHeader *block,
                         size_t requested_size) {
  if (!pool || !block || !validate_block(block))
    return NULL;
  ...
  BlockHeader *new_block =
      (BlockHeader *)((uint8_t *)block + sizeof(BlockHeader) + aligned_size);
...
BlockHeader *coalesce_blocks(MemoryPool *pool, BlockHeader *block) {
  ...
  while (block->prev && block->prev->is_free && validate_block(block->prev)) {
    ...
    // Check if blocks are adjacent
    uint8_t *prev_end =
        (uint8_t *)prev_block + sizeof(BlockHeader) + prev_block->size;
    if (prev_end == (uint8_t *)block) {
      ...
      if (pool->enable_debugging) {
        log_message("DEBUG", "Coalesced with previous: new size %zu",
                    block->size);
      }
    } else {
      break;
 ...
 */
void remove_from_free_list(MemoryPool *pool, BlockHeader *block,
                           size_t list_index) {
  if (!pool || !block || list_index >= MAX_FREE_LISTS)
    return;
...
void memory_pool_free(MemoryPool *pool, void *ptr) {
  ...
  if (!validate_block(block)) {
    log_message("ERROR", "Pool '%s': Invalid block header for free",
                pool->name);
    return;
  }
...
bool memory_pool_get_stats(MemoryPool *pool, PoolStats *stats) {
  ...
  if (pool->total_size > 0) {
    stats->fragmentation_ratio =
        (pool->num_blocks * 100) / (pool->total_size / sizeof(BlockHeader));
  }
  ...
  for (size_t i = 0; i < MAX_FREE_LISTS; i++) {
    ...
    for (BlockHeader *block = pool->free_lists[i].head; block;
         block = block->next) {
...
void display_pool_stats(MemoryPool *pool) {
  ...
  PoolStats stats;
  if (!memory_pool_get_stats(pool, &stats))
    return;

  printf("\n=== Memory Pool Statistics: %s ===\n", pool->name);
  printf("Total size: %zu bytes\n", pool->total_size);
  printf("Used size: %zu bytes (%.1f%%)\n", pool->used_size,
         (double)pool->used_size * 100.0 / pool->total_size);
  printf("Peak usage: %zu bytes (%.1f%%)\n", pool->peak_usage,
         (double)pool->peak_usage * 100.0 / pool->total_size);
...
void dump_pool_layout(MemoryPool *pool) {
  ...
  while (current && validate_block(current)) {
    printf("Block %zu: %s, Size: %zu bytes, Address: %p\n", block_num++,
           current->is_free ? "FREE" : "USED", current->size, (void *)current);
...
void memory_pool_destroy(MemoryPool *pool) {
  ...
  if (pool->num_allocations != pool->num_deallocations) {
    log_message(
        "WARN",
        "Pool '%s': Memory leak detected - %zu allocations, %zu deallocations",
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "--help") == 0) {
      ...
      pool_size = size;
               ...
               strcmp(argv[i], "--alignment") == 0) {
      if (++i >= argc) {
        printf("Error: Alignment value required\n");
        return 1;
      }
      int align;
      if (!str_to_int(argv[i], &align) || align <= 0 ||
          (align & (align - 1)) != 0) {
