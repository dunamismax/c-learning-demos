...
void bubble_sort(void* array, size_t size, size_t element_size,
    ...
    if (!array || size <= 1 || !compare) return;
 ...
 */
static size_t partition(void* array, size_t low, size_t high, size_t element_size,
                        int (*compare)(const void* a, const void* b)) {
...
static void quick_sort_recursive(void* array, size_t low, size_t high,
                                 size_t element_size, int (*compare)(const void* a, const void* b)) {
    if (low < high) {
...
static void merge(void* array, size_t left, size_t mid, size_t right,
    ...
    memcpy(left_array, get_element(array, left, element_size), left_size * element_size);
    memcpy(right_array, get_element(array, mid + 1, element_size), right_size * element_size);
...
size_t linear_search(const void* array, size_t size, size_t element_size,
    ...
    if (!array || !target || !compare) return SIZE_MAX;
